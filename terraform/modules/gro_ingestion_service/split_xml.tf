resource "aws_iam_role" "split_xml_lambda" {
...
data "aws_iam_policy_document" "split_xml_lambda" {
  statement {
    sid = "LambdaKMSAccess"
    actions = [
      "kms:Decrypt",
...
resource "aws_iam_role_policy_attachment" "split_xml_lambda" {
  policy_arn = aws_iam_policy.split_xml_lambda.arn
  role       = aws_iam_role.split_xml_lambda.name
}

resource "aws_iam_role_policy_attachment" "split_xml_lambda_xray_access" {
...
resource "aws_cloudwatch_log_group" "split_xml_log" {
  name              = "/aws/lambda/${aws_lambda_function.split_xml_lambda.function_name}"
  retention_in_days = var.cloudwatch_retention_period

  kms_key_id = aws_kms_key.gro_ingestion.arn
}

resource "aws_lambda_function" "split_xml_lambda" {
