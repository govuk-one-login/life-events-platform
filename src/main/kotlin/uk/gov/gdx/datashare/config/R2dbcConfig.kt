...
class R2dbcConfig(
  ...
  override fun connectionFactory(): ConnectionFactory {
    ...
    val stubOptions = options.mutate()
      .option(
        ConnectionFactoryOptions.USER, "placeholder"
      ).option(
        ConnectionFactoryOptions.PASSWORD, "placeholder"
      ).build()
    ...
    val connectionPublisher: Mono<out Connection?> = Mono.defer {
      val optionsToUse = options.mutate().option(
        ConnectionFactoryOptions.USER, dbCredentialsService.username()
      )
        .option(
          ConnectionFactoryOptions.PASSWORD, dbCredentialsService.password()
        )
        .option(
          ConnectionFactoryOptions.SSL, dbSslRequired
        )
        .build()
